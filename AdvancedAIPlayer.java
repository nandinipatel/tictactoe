...
{
    public AdvancedAIPlayer(final board board, final int n, final String s) {
        super(board, n, s);
    }
    
    public void play(final board board) {
        final int senseWin = this.senseWin();
        if (senseWin != 0) {
            board.makeMove(senseWin, this.playerSymbol);
            System.out.println(this.playerName + ": yes... I see a winning move at " + senseWin + "!");
            return;
        }
        final int senseLoss = this.senseLoss();
        if (senseLoss != 0) {
            board.makeMove(senseLoss, this.playerSymbol);
            System.out.println(this.playerName + ": I am playing defense at " + senseLoss);
            return;
        }
